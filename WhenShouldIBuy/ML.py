...
plt.pyplot.show()
import pandas as pd
pd.DataFrame(df).to_csv('joinDf.csv', header=False)

# prepare data
from sklearn.model_selection import train_test_split
X_data = df[['premium', 'date_delta']]
Y_data = df[['price']]
...
plt.scatter(X_data[['date_delta']], Y_data, color='red')
...
plt1.scatter(X_train[['date_delta']], y_train)
...
# poly_regr.fit(X_poly, Y_data)
X_data_1 = df.loc[200, ['premium', 'date_delta']]
X_data_all = df.loc[:, ['premium', 'date_delta']]
print('X_data_all', X_data_all)
# y_pred = regr.predict(X_test)

# print('X_data_1', X_data_1)
y_pred_100 = regr.predict(X_data_1.values.reshape(-1, 2))
y_pred_all = regr.predict(X_data_all.values.reshape(-1, 2))
...
percent_treshold = int(X_data_begining * 0.7)
...
for i in range(y_pred_all.size):
    if y_pred_all[i] <= percent_treshold:
        print(i, '70% to: ', y_pred_all[i])
        break

# print(df.loc[77, 'price'])
# print(y_pred_all[77])

# percent_treshold: 158
# X_data_begining: 264.91
# 662 60% to:  [157.94992364]
